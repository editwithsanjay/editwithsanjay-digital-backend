...
export async function CashFreeOnlinePaymentInitiatorController(req,res)
{
  ...
  try {
    ...
    // Create a Cashfree order with dynamic customer details
    const cashfreeOrder = await generateCashFreeOrder({
        ...
        "customer_details": {
            ...
            "customer_name": customer_details?.customer_id || order_id
        },
        "order_meta": {
    ...
    const payload = list_items?.map((el) => ({
      ...
      paymentId: "", // Will update after successful payment
      payment_status: "PENDING",
      delivery_address: addressId, // Fixed: Use the actual addressId
      subTotalAmt: subTotalAmt,
